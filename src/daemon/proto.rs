...
/// daemon emits".
///
/// The filter is compared by exact path equality — no prefix or
/// subdirectory matching. `git` events carry the repo's canonicalized
/// workdir root, so to follow a repo the filter must be that canonical
/// root (not a subdirectory inside it); `cmd` events carry the
/// directory the command ran in. An event without a cwd never matches
/// a filter.
