...
fn subscribe_receives_git_event_from_real_daemon() {
    ...
    // STATUS once to make the daemon discover + watch the repo. We do
    // this through `chevron git` (which calls status_for_cwd). The
    // widened socket budget matters most HERE: with the production
    // 10 ms query timeout this registration silently fell back to
    // inline compute under load, the daemon never learned about the
    // repo, and no amount of HEAD-touching below could raise an event.
    daemon
        .cmd()
        .arg("git")
